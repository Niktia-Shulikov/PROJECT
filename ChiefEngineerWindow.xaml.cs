...
namespace AGSS
{
    ...
    public partial class ChiefEngineerWindow : Window
    {
        ...
        public ChiefEngineerWindow()
        {
            InitializeComponent();
            LoadProjects();
            AddCombo.ItemsSource = new List<string> { "Площадь", "Координаты площади", "Профиль", "Координаты профиля", "Канал", "Полет", "Спектрометер", "Метаданые" };
        }
                ...
                switch (AddCombo.SelectedItem.ToString())
                {
                    ...
                    case "Канал":
                        ...
                        else
                        {
                            ...
                            {
                                MagnetChannel channel = new MagnetChannel { MeasurementResult = double.Parse(oneBox.Text.Trim().Replace('.', ',')), ProfileCoordinatesId = int.Parse(twoBox.Text.Trim()) };
                ...
                using (var context = new GravitySurveyOnDeleteNoAction())
                {
                    ...
                    for (int i = 0; i <= random.Next(1, 10); i++)
                    {
                        ...
                        for (int j = 0; j <= random.Next(2, 9); j++)
                        {
                            ...
                            MagnetChannel channel1 = new MagnetChannel { ProfileCoordinatesId = profileCoordinate.ProfileCoordinatesId, MeasurementResult = random.Next(50, 1001) };

                            context.MagnetChannels.Add(channel1);
                            context.SaveChanges();
                        }
                    }
